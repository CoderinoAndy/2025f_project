...
def _footer_heavy_concert_email():
    return {
        "sender": "Spotify <notifications@spotify.example>",
        "title": "Bryant Barnes in Toronto.",
        "body": (
            "GET TICKETS\n\n"
            "Grab your tickets to Bryant Barnes at The Opera House on Sat, Mar 21 now. "
            "The clock is ticking!\n\n"
            "Spotify may earn affiliate commissions from ticket sales.\n\n"
            "Edit your profile to stop receiving these emails.\n\n"
...
def _brevite_bundle_email():
    return {
        "sender": '"BrevitÄ“" <support@brevite.co>',
        ...
        "body": (
            ...
            "You received this email from BrevitÄ“. If you would like to unsubscribe, click here."
        ),
        "recipients": "",
        "cc": "",
    }
...
def _uber_expiring_promo_email():
    return {
        "sender": "Uber Eats <uber@uber.com>",
        "title": "Your 40% off promo expires soon",
        "body": (
            "Get the food you love with your Uber Eats promo.\n\n"
            ...
            "Get 4 weeks free\n\n"
            "Use your 40% off promo before it's gone!\n\n"
            "Order now\n\n"
            "*Terms and conditions apply. Get 40% off your next order. Valid until 16/3/2026.\n\n"
            "Help Centre\n\n"
            "Unsubscribe\n\n"
            "Terms\n\n"
            "Privacy\n\n"
...
def _many_section_update_email():
    sections = [
        (
            f"Section {index:02d} covers milestone {index} and records the owner, deadline, "
            f"dependencies, and follow-up work needed to keep the rollout on track."
        )
        for index in range(1, 13)
    ]
    return {
        "sender": "Program Office <program@example.com>",
        "title": "Quarterly rollout tracker",
        "body": "\n\n".join(sections),
        "recipients": "",
        "cc": "",
    }
...
class OllamaSummaryFormattingTests(unittest.TestCase):
    ...
    def test_clean_body_for_prompt_stops_at_footer_block_after_real_content(self):
        cleaned = ollama_client._clean_body_for_prompt(_footer_heavy_concert_email(), max_chars=800)

        self.assertIn("Bryant Barnes", cleaned)
        self.assertNotIn("affiliate commissions", cleaned.lower())
        self.assertNotIn("edit your profile", cleaned.lower())
        self.assertNotIn("stop receiving these emails", cleaned.lower())
    ...
    def test_promo_summary_ignores_legal_bullet_and_surfaces_primary_offer(self):
        summary = ollama_client._bulk_newsletter_summary(_uber_expiring_promo_email())

        self.assertIsNotNone(summary)
        self.assertIn("40% off the next order", summary)
        self.assertIn("4-week free trial", summary)
        self.assertNotIn("Terms and conditions apply", summary)
    ...
    def test_body_for_context_keeps_later_sections_for_long_emails(self):
        summary_context = ollama_client._body_for_context(
            _many_section_update_email(),
            max_chars=9000,
            max_sentences=20,
        )

        self.assertIn("Section 01", summary_context)
        self.assertIn("Section 12", summary_context)
    ...
    def test_summary_looks_unusable_flags_footer_focused_summary(self):
        email = _footer_heavy_concert_email()
        email["summary"] = (
            "The email says Spotify may earn affiliate commissions from ticket sales and that "
            "the recipient can edit the recipient's profile to stop receiving these emails."
        )
    ...
    @mock.patch(
        "app.ollama_client._call_ollama",
        return_value="Order now to sign up for Uber One and get 4 weeks free.",
    )
    def test_summarize_email_promo_fallback_uses_offer_not_legal_or_cta_copy(self, _mock_call):
        summary = ollama_client.summarize_email(_uber_expiring_promo_email(), email_id="uber-promo-1")

        self.assertIsNotNone(summary)
        self.assertIn("40% off", summary)
        self.assertIn("next order", summary)
        self.assertIn("4-week free trial", summary)
        self.assertNotIn("Terms and conditions apply", summary)
        self.assertNotIn("Order now", summary)
        self.assertNotIn("the the recipient's", summary)

    @mock.patch(
        "app.ollama_client._call_ollama",
        return_value="A promotional update from Uber Eats. It advertises 40% off your next order.",
    )
    def test_summarize_email_promo_prompt_adds_legal_disclaimer_constraint(self, mock_call):
        summary = ollama_client.summarize_email(_uber_expiring_promo_email(), email_id="uber-promo-prompt-1")

        self.assertIsNotNone(summary)
        summarize_call = next(
            call for call in mock_call.call_args_list if call.kwargs.get("task") == "summarize"
        )
        system_prompt = summarize_call.kwargs["messages"][0]["content"]
        user_prompt = summarize_call.kwargs["messages"][1]["content"]
        self.assertIn("Do not lead with legal disclaimers", system_prompt)
        self.assertIn("summarize the primary offer or deal", user_prompt)

    @mock.patch(
        "app.ollama_client._call_ollama",
        return_value=(
            "The kit includes the Jumper 2026 bag, which has been redesigned based on feedback "
            "from over 10,000 photographers."
        ),
    )
    @mock.patch("app.ollama_client._vision_user_message", side_effect=_mock_vision_message)
    def test_summarize_email_prefers_richer_promo_fallback_when_model_is_too_terse(
        self,
        _mock_vision,
        _mock_call,
    ):
        summary = ollama_client.summarize_email(_brevite_bundle_email(), email_id="promo-bundle-1")

        self.assertIsNotNone(summary)
        self.assertTrue(summary.startswith("A promotional update from"))
        self.assertIn("camera top insert", summary.lower())
        self.assertIn("packable rain cover", summary.lower())
        self.assertIn("$41 off", summary)
        self.assertIn("lifetime warranty", summary.lower())
    ...
    @mock.patch("app.ollama_client._vision_user_message", side_effect=_mock_vision_message)
    def test_summarize_email_rejects_footer_focused_model_output(self, _mock_vision, _mock_call):
        summary = ollama_client.summarize_email(
            _footer_heavy_concert_email(),
            email_id="spotify-footer-1",
        )

        self.assertIsNotNone(summary)
        self.assertNotIn("affiliate commissions", summary.lower())
        self.assertNotIn("profile", summary.lower())
        self.assertTrue(
            any(term in summary for term in ("Bryant Barnes", "Opera House", "Mar 21"))
        )
